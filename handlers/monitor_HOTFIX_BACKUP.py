...
def tp2_msg(pair_clean, emoji, direction, tp2, profit_lev, lev, fmt):
                            pos["tp2_sent"] = True
                            pos["tp3_sent"] = pos.get("tp3_sent", False)
    return (
        f"🎯 <b>TP2 HIT!</b>\n\n"
        f"📌 <b>Pair</b>      : #{pair_clean}USDT\n"
        f"📊 <b>Direction</b> : {emoji} {direction}\n"
        ...
        try:
            ...
            for key, pos in positions.items():
                ...
                async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                    ...
                    if direction == 'LONG':
                        if not pos.get("tp1_hit", False) and current_price >= tp1:
                            pos["tp1_hit"] = True
                            pos["tp1_sent"] = True
                            pos["be_active"] = True
                            pos["sl"] = entry
                        ...
                        elif pos.get("tp2_sent", False) == False and  current_price >= tp2 and pos.get("tp2_sent", False) == False:
                            pos["tp3_sent"] = pos.get("tp3_sent", False)
                            pos["tp2_hit"] = True
                            save_positions(positions)
                        ...
                        if not pos.get("tp1_hit", False) and current_price <= tp1:
                            ...
                            update_signal_status(pair_clean, tp_hit=1)
                            await app.bot.send_message(user_id, tp1_msg(pair_clean, emoji, direction, tp1, profit_lev, lev, fmt), parse_mode="HTML")
                        elif pos.get("tp2_sent", False) == False and current_price <= tp2:
                            pos["tp3_sent"] = pos.get("tp3_sent", False)
                            pos["tp2_hit"] = True
                            save_positions(positions)
